...
<head>
<meta charset="utf-8">
<title>Hire Finishing List</title>
<style>
...
<body>
  <h1 class="header1">Cab Booking List to Finish Hires</h1>

  <div class='menu-box'>
    <table border="1">
      <%
        // Fetch cab details from the service
        List<Booking> bookings = BookingService.getAllHireFinishingCabs();

        // Debugging: Check if data is retrieved
        if (bookings == null || bookings.isEmpty()) {
          %>
            <script>
                alert("No Hires Found to Finish.");
      ...
      <tr>
        ...
        <td><form action="finishTheJourney" method="POST">
            		<button type="submit" class="read-more-button">Finish Journey</button>
            		<input type="hidden" name="JourneyStatus" value="Your Journey is Finished" />
            		<input type="hidden" name="BookingID" value="<%= booking.getBooking_ID() %>" />
            		<input type="hidden" name="CabID" value="<%= booking.getCab_ID() %>" />
        </form></td>
      </tr>
      <% } %>
      <% } // End else block %>
